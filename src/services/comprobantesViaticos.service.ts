export async function subirComprobanteViatico(id_viatico: number, file: File, token?: string) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "/api"}/comprobantes-viaticos/subir`, {
    method: 'POST',
    body: formData,
    headers,
  });
...
export async function getComprobantesPorViatico(id_viatico: number, token?: string) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "/api"}/comprobantes-viaticos/${id_viatico}`, {
    headers
  });
