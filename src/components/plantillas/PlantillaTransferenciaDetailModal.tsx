...
import { X, ArrowRightLeft, DollarSign, Building2, FileText, ExternalLink, FileCheck, CreditCard, User } from 'lucide-react';
...
// Interfaz para una cuenta de transferencia individual
export interface CuentaTransferencia {
  beneficiario: string;
  tipo_cuenta: string; // 'cuenta', 'clabe', 'tarjeta'
  numero_cuenta: string;
  banco_destino: string;
  monto: string | number;
  tipo_tarjeta?: string; // 'debito', 'credito'
}

// Tipos específicos para la plantilla de Transferencias
export interface SolicitudTransferenciaData {
  id_solicitud: number;
  asunto: string;
  cuentas_transferencia: CuentaTransferencia[]; // array de cuentas de transferencia
...
interface PlantillaTransferenciaDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  solicitud: SolicitudTransferenciaData;
  titulo?: string;
}

export const PlantillaTransferenciaDetailModal: React.FC<PlantillaTransferenciaDetailModalProps> = ({
  isOpen,
  onClose,
  solicitud,
  titulo = "REGRESOS - TRANSFERENCIA"
}) => {
  ...
  const getTipoCuentaLabel = (tipoCuenta: string, tipoTarjeta?: string): string => {
    ...
    let label = tipos[tipoCuenta.toLowerCase()] || tipoCuenta;
    
    if (tipoCuenta.toLowerCase() === 'tarjeta' && tipoTarjeta) {
      const tipoFormatted = tipoTarjeta === 'debito' ? 'Débito' : tipoTarjeta === 'credito' ? 'Crédito' : tipoTarjeta;
      label += ` - ${tipoFormatted}`;
    }
  ...
  const calcularMontoTotal = (): number => {
    return solicitud.cuentas_transferencia.reduce((total, cuenta) => {
      const monto = typeof cuenta.monto === 'string' ? parseFloat(cuenta.monto) : cuenta.monto;
      return total + (monto || 0);
    }, 0);
  };
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[96vh] flex flex-col border border-blue-100">
        ...
        <div className="flex flex-col lg:flex-row gap-6 overflow-y-auto max-h-[96vh] p-4 sm:p-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            {solicitud.cuentas_transferencia && solicitud.cuentas_transferencia.length > 0 && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-blue-900 mb-4 pb-2 border-b border-blue-200 flex items-center gap-2">
                  <CreditCard className="w-5 h-5 text-blue-500" />
                  Cuentas de Transferencia ({solicitud.cuentas_transferencia.length})
                </h3>
                <div className="grid grid-cols-1 gap-4">
