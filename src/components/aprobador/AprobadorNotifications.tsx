...
export default function Notifications({ open, onClose }: NotificationsProps) {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/notificaciones`, {
  ...
  const markAsRead = async (id: number) => {
    const token = getAuthToken();
    await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/notificaciones/${id}/marcar-leida`, {
      method: "POST",
      headers: { Authorization: token ? `Bearer ${token}` : '' }
    });
    await fetchNotifications();
