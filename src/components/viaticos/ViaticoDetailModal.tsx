...
function ConfirmDeleteModal({ open, onConfirm, onCancel }: { open: boolean, onConfirm: () => void, onCancel: () => void }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full border border-red-200 flex flex-col items-center">
        <div className="mb-4 text-red-600">
          <CheckCircle className="w-12 h-12 mx-auto text-red-400" />
        </div>
        <h2 className="text-xl font-bold text-red-700 mb-2 text-center">¿Eliminar comprobante de gasto?</h2>
        <p className="text-gray-700 mb-6 text-center">Esta acción no se puede deshacer. ¿Seguro que deseas eliminar este comprobante?</p>
        <div className="flex gap-4 w-full justify-center">
          <button onClick={onCancel} className="px-4 py-2 rounded-lg border border-gray-300 bg-gray-100 text-gray-700 hover:bg-gray-200 font-semibold">Cancelar</button>
          <button onClick={onConfirm} className="px-4 py-2 rounded-lg bg-red-600 text-white font-bold hover:bg-red-700 shadow">Eliminar</button>
        </div>
      </div>
    </div>
  );
...
export function ViaticoDetailModal({ isOpen, viatico, onClose }: ViaticoDetailModalProps) {
  ...
  // (Eliminada función duplicada, solo se usa el modal visual)

  // Abre el modal y guarda el id a eliminar
  const handleAskDeleteGastoComprobante = (id_comprobante: number) => {
    setComprobanteToDelete(id_comprobante);
    setDeleteModalOpen(true);
  };

  // Confirma y elimina
  const handleConfirmDeleteGastoComprobante = async () => {
    if (!comprobanteToDelete) return;
    try {
      const token = getAuthToken();
      await ComprobantesGastoViaticoService.delete(comprobanteToDelete, token);
  ...
  return (
    <>
      <ConfirmDeleteModal
        ...
        onConfirm={handleConfirmDeleteGastoComprobante}
        onCancel={() => { setDeleteModalOpen(false); setComprobanteToDelete(null); }}
      />
                    ...
                    {gastoComprobantes.map((comp) => (
                      <div key={comp.id_comprobante_gasto || comp.id_comprobante} className="bg-white p-4 rounded-xl border border-green-100 shadow flex flex-col items-center">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => handleAskDeleteGastoComprobante(comp.id_comprobante_gasto || comp.id_comprobante)}
