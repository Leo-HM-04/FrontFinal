...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  // Memoizar estilos de fondo para optimizar rendimiento
  ...
  // Definir elementos de navegación con sus propiedades
  const navItems = [
    { href: '/dashboard/admin', label: 'Inicio', icon: Home, description: 'Panel principal del administrador' },
    {
    ...
    {
      href: '/dashboard/admin/recurrentes',
      label: 'Solicitudes Recurrentes',
      icon: Repeat,
      description: 'Gestionar solicitudes periódicas',
    },
    {
      href: '/dashboard/admin/graficas',
      label: 'Gráficas',
      icon: BarChart2,
    ...
    },
    {
      ...
      label: 'Mi Perfil',
      icon: User,
      description: 'Configurar perfil de administrador',
    },
  ];

  // Manejador para cerrar el menú lateral
  ...
  // Desbloquear audio global al primer clic del usuario
  useEffect(() => {
    const unlockAudio = () => {
      if (audioRef.current) {
        audioRef.current.play().catch((error) => {
          console.error('Error al intentar desbloquear audio:', error);
        });
      }
  ...
  // Configurar listeners para clics fuera y tecla Escape
  useEffect(() => {
          ...
          {isMenuOpen && (
            <div className="fixed inset-0 z-50 overflow-hidden">
              ...
              <motion.aside
                ...
                initial={{ x: -320, opacity: 0, clipPath: 'inset(0 40% 0 0)' }}
                animate={{
                  x: 0,
                  opacity: 1,
              ...
              >
                ...
                <div
                  className="flex flex-col items-start gap-2 pt-8 pb-2 px-8"
                  style={{ background: 'linear-gradient(135deg, #0057D9 0%, #004AB7 100%)' }}
                >
                  <div className="w-full bg-white/90 rounded-xl border border-blue-200 flex items-center gap-4 px-4 py-3 mb-4 shadow-sm">
                    ...
                    <div className="flex flex-col items-start justify-center">
                      <span className="font-bold text-blue-800 text-base leading-tight">
                    ...
                    return (
                      <Link
                        ...
                        className={`relative flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 group text-base font-medium select-none
                          ${isActive ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-gray-700 hover:bg-blue-50 hover:text-blue-600'}`}
                        onClick={closeMenu}
                      ...
                      >
                        {isActive && (
                          <span
                            ...
                            style={{ boxShadow: '0 0 6px #2563eb55' }}
                          ></span>
                        )}
                        <item.icon
                          className={`w-5 h-5 transition-transform group-hover:scale-110 ${isActive ? 'text-blue-700' : ''}`}
                        />
                      ...
                      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                          <Dialog.Panel className="w-full max-w-sm rounded-2xl bg-white p-8 shadow-2xl border border-red-200 flex flex-col items-center">
                            ...
                            <p className="text-red-900/90 text-base mb-4 text-center">
                              ¿Estás seguro de que deseas cerrar tu sesión? <br />
                              Se cerrará tu acceso como administrador y necesitarás iniciar sesión nuevamente para
                              continuar.
                            </p>
