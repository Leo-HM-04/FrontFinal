...
import { LogOut, Home, User, Menu, ClipboardList, Repeat, Briefcase, FileText } from 'lucide-react';
...
export function SolicitanteLayout({ children }: SolicitanteLayoutProps) {
  ...
  const navItems = [
    ...
    { href: '/dashboard/solicitante/nueva-solicitud', label: 'Nueva Solicitud', icon: <FileText className="w-5 h-5" /> },
    { href: '/dashboard/solicitante/mis-solicitudes', label: 'Mis Solicitudes', icon: <ClipboardList className="w-5 h-5" /> },
    { href: '/dashboard/solicitante/nuevo-viatico', label: 'Nuevo Viático', icon: <FileText className="w-5 h-5" /> },
    { href: '/dashboard/solicitante/mis-viaticos', label: 'Mis Viáticos', icon: <ClipboardList className="w-5 h-5" /> },
  ...
  return (
    <>
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 pt-20 min-h-[calc(100vh-4rem)]">
        {children}
      </main>
      </div>
    </>
